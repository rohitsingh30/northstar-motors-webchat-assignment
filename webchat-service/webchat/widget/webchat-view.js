...
function vehicleCard(item) {
  ...
  if (/^veh-[0-9]{3}$/.test(item.id || "")) {
    ...
    link.addEventListener("click", (event) => {
      ...
      // An integrated host may handle navigation without a page reload. The href remains the
      // accessible fallback when no host cancels this event.
...
function vehicleAvailabilityCard(view) {
...
function confirmationCard(view) {
  ...
  Object.entries(view.summary || {}).forEach(([key, value]) => {
    ...
    const displayValue = key === "dealershipId"
      ...
      : key === "vehicleId"
        ? [view.vehicle?.year, view.vehicle?.make, view.vehicle?.model, view.vehicle?.variant]
          .filter(Boolean)
          .join(" · ") || "Selected vehicle"
...
function businessInformationCard(view) {
  const card = document.createElement("article");
  card.className = "webchat-card";
...
function comparisonTable(items) {
  ...
  rows.forEach(([label, value]) => {
    ...
    items.slice(0, 3).forEach((vehicle) => {
      row.append(textElement("td", "", value(vehicle)));
    });
    body.append(row);
  });
...
function richSummary(message) {
  ...
  if (message.viewType === "slot_list") return "Available workshop times are shown below.";
...
function suggestionChips(suggestions, { allowMany = false, className = "" } = {}) {
...
export function renderMessage(message) {
  ...
  // Rich vehicle results already contain the facts in cards. Keep only a short
  ...
  ].includes(message.viewType);
  item.append(messageContent(message, structured ? richSummary(message) : message.text));

  if (message.viewType === "vehicle_list" && Array.isArray(message.view?.items)) {
    const cards = document.createElement("div");
    cards.className = "webchat-cards";
    message.view.items
      .slice(0, 3)
      .forEach((vehicle) => cards.append(vehicleCard(vehicle)));
    item.append(cards);
  } else if (message.viewType === "vehicle_availability" && message.view) {
    item.append(vehicleAvailabilityCard(message.view));
  } else if (message.viewType === "vehicle_comparison" && Array.isArray(message.view?.items)) {
    item.append(comparisonTable(message.view.items));
  } else if (message.viewType === "test_drive_slot_picker" && message.view) {
    item.append(testDriveSlotPicker(message.view));
  } else if (message.viewType === "service_list" && Array.isArray(message.view?.items)) {
    const cards = document.createElement("div");
    cards.className = "webchat-cards webchat-service-list";
    message.view.items.slice(0, 12).forEach((service) => cards.append(serviceCard(service)));
    item.append(cards);
  } else if (message.viewType === "slot_list" && message.view) {
    const flow = document.createElement("div");
    flow.className = "webchat-booking-flow webchat-workshop-flow-standalone";
    flow.dataset.workshopFlow = "true";
    flow.workshopOptions = message.view;
    flow.append(workshopSlotPicker(message.view));
    item.append(flow);
  } else if (
    ["offer_list", "dealership_list", "workshop_location_list", "opening_hours"].includes(
      message.viewType,
    ) && Array.isArray(message.view?.items)
  ) {
    const cards = document.createElement("div");
    cards.className = "webchat-cards";
    if (message.viewType === "opening_hours") {
      message.view.items.slice(0, 8).forEach((value) => cards.append(openingHoursCard(value)));
    } else {
      message.view.items.slice(0, 8).forEach((value) => cards.append(
        message.viewType === "offer_list" ? offerCard(value) : dealershipCard(value),
      ));
    }
    item.append(cards);
    if (message.viewType === "offer_list" && message.view.financeNotice) {
      item.append(textElement("small", "", message.view.financeNotice));
    }
  } else if (message.viewType === "confirmation" && message.view?.draftId) {
    item.append(confirmationCard(message.view));
  } else if (message.viewType === "draft" && message.view?.draftId) {
    item.append(draftCard(message.view));
  } else if (message.viewType === "private_booking_lookup") {
    item.append(privateLookupForm(message.view || {}));
  } else if (message.viewType === "workshop_booking_details" && message.view) {
    item.append(workshopBookingDetailsCard(message.view));
  } else if (message.viewType === "part_exchange_estimate_form") {
    item.append(partExchangeEstimateForm(message.view || {}));
  } else if (message.viewType === "receipt" && message.view) {
    item.append(receiptCard(message.view));
  } else if (message.viewType === "part_exchange_estimate" && message.view) {
    item.append(partExchangeEstimateCard(message.view));
  }

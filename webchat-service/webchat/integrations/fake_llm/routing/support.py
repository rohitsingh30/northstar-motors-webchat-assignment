...
class SupportRouter:
    ...
    def _offers(context: ConversationContext) -> ProviderReply | None:
        words = context.words
        explicit_offers = words.intersection({"offer", "offers", "deal", "deals"})
    ...
    def _callback(context: ConversationContext) -> ProviderReply | None:
        ...
        fields = contextual_form_prefill(
            ...
            fields,
            context.latest,
            _prior_customer_messages(context),
        )
    ...
    def _departments(context: ConversationContext) -> ProviderReply | None:
        if not context.words.intersection({"department", "departments"}):
            return None
        if town := location_query(context.latest):
            return tool(
                "local-departments-at-location",
                "find_dealership_departments",
                {"town": town},
            )
        plural_scope = bool(context.words.intersection({"all", "each", "dealerships", "dealers"}))
        if plural_scope or not context.displayed_dealership_ids:
            return tool("local-departments", "list_dealership_departments")
        if len(context.displayed_dealership_ids) == 1:
            return tool(
                "local-departments-at-location",
                "find_dealership_departments",
                {"dealershipId": context.displayed_dealership_ids[0]},
            )
        return clarify("Which dealership do you mean?", "get_dealership", fields=("id",))

    @classmethod
    def _business_information_request(cls, context: ConversationContext) -> ProviderReply | None:
        if cls._business_information(context.words):
            ...
            return tool(
                ...
                {"topic": topic, "question": context.latest},
            )
        return None

    @classmethod
    def _part_exchange_request(cls, context: ConversationContext) -> ProviderReply | None:
    ...
    def _sales_enquiry(context: ConversationContext) -> ProviderReply | None:
        ...
        if not (
            ...
            or ("question" in words and words.intersection({"buying", "buy", "car", "vehicle"}))
        ):
            return None
    ...
    def _opening_hours(context: ConversationContext) -> ProviderReply | None:
        ...
        if "holiday" in words:
            if town := location_query(context.latest):
                arguments["town"] = town
            return tool("local-holiday-opening-hours", "list_holiday_opening_hours", arguments)
        ...
        singular_reference = bool(
            context.words.intersection({"dealership", "dealer", "it", "that"})
        )
        if (
            "town" not in arguments
            and singular_reference
            and len(context.displayed_dealership_ids) > 1
        ):
            return clarify(
                "Which dealership do you mean?",
                "list_opening_hours",
                preconditions=("single_dealership_or_all",),
            )
    ...
    def _locations(context: ConversationContext) -> ProviderReply | None:
        ...
        if contact_question or dealership_question:
            arguments = (
                {"town": location_query(context.latest)} if location_query(context.latest) else {}
            )
    ...
    def _business_information(words: frozenset[str]) -> bool:
        ...
        estimate_notice = (
            ...
            and bool(words.intersection({"calculated", "calculation", "calculate", "methodology"}))
        )
    ...
    def _part_exchange(
        ...
        estimate_fields = {
            key: fields[key] for key in ("registration", "mileage", "condition") if key in fields
        }
...
def _prior_customer_messages(context: ConversationContext) -> list[str]:
    ...
    for message in reversed(context.messages):
        ...
        if not latest_skipped:
            latest_skipped = True
            continue
        selected.append(str(message.get("content") or ""))
        if len(selected) >= 6:
            break

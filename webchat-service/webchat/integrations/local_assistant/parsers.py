...
def location_query(text: str) -> str | None:
    patterns = (
        ...
        r"^\s*(?!(?:show|find|list|where|which|what|our|your|the)\b)([a-z][a-z -]{1,60}?)\s+dealership\b",
    )
    match = next(
...
def vehicle_filters(text: str) -> dict[str, Any]:
    ...
    if "lowest mileage" in text or "low mileage" in text:
        filters["sort"] = "mileageAsc"
    elif "highest price" in text:
...
def _apply_vehicle_dimensions(filters: dict[str, Any], text: str) -> None:
    ...
    for field, values in dimensions.items():
        match = next(
            (value for value in values if re.search(rf"\b{value}\b", text, re.IGNORECASE)),
            None,
        )
        if match:

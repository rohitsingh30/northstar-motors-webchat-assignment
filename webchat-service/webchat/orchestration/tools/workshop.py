...
class WorkshopReadToolHandler:
    ...
    async def _service_information(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        return ToolResult(
            f"{name} is {price}{duration_text}." + (f" {description}" if description else ""),
            None,
            None,
            {
    ...
    async def _test_drive_slots(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if availability.get("availability") != "available":
            return await vehicle_availability_result(self.dealership, vehicle_id, availability)
    ...
    async def _workshop_slots(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if not service_name and not filters.get("serviceTypeId"):
            ...
            if dealership_id:
                ...
                items = [
                    ...
                    if str(service.get("id")) in available_service_ids
                ]
            return _service_selection_result(
                ...
                (
                    ...
                    if dealership_id
                    else "Please choose a workshop service before selecting a time."
                ),
        ...
        if not data["items"]:
            ...
            alternative_data = await self.dealership.list_workshop_slots(alternative_filters)
...
def _future_slots(items: list[dict[str, Any]], now: datetime) -> list[dict[str, Any]]:
    return [
        item
        for item in items
        if item.get("startsAt") and datetime.fromisoformat(str(item["startsAt"])) > now
    ]
...
def _service_selection_result(
    ...
    return ToolResult(
        ...
        "service_list",
        payload,
        {"items": items, **({"dealershipId": dealership_id} if dealership_id else {})},
    )

...
class ConversationContext:
    ...
    has_active_workshop_service: bool
    ...
    def from_messages(cls, messages: list[dict[str, Any]]) -> ConversationContext:
        ...
        workflow_domain = _developer_value(
            messages,
            "Active application workflow state (trusted data): ",
            "domain",
            r"[a-z][a-z_]{0,39}",
        )
        workflow_service = _developer_value(
            messages,
            "Active application workflow state (trusted data): ",
            "serviceTypeId",
        ...
        return cls(
            ...
            active_offer=page_offer or (displayed_offer_ids[0] if len(displayed_offer_ids) == 1 else None),
            active_vehicle=(
                explicit_ids[-1]
                if explicit_ids
            ...
            has_active_workshop_service=bool(
                workflow_domain == "workshop" and workflow_service
            ),

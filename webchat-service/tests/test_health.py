...
def test_service_hosts_the_widget_bundle(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(settings)) as client:
        ...
        form_submit_response = client.get("/widget/core/form-submit.js")
    ...
    assert form_submit_response.status_code == 200
    assert "createFormSubmitHandler" in form_submit_response.text
    ...
    assert "testDriveSlotPicker" in appointment_view_response.text
